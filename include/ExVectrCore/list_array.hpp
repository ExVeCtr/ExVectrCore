...
/**
 * This is a dynamic array. If full, the size will be doubled. Uses heap memory
 * and can be slower. Iterating through list is fast and item access is O(1).
 * Removing or adding an item is O(n). The next item index is always +1. So the
 * same as a normal array. The sizeControl setting on construction can cause
 * heap fragmentation if set to true but will reduce size if unneeded
 * @param TYPE type of data to store in ListArray
 */
template <typename TYPE> class ListArray : public List<TYPE> {
private:
  // Current max size.
  size_t maxSize_ = 0;
...
public:
  /**
   * @param sizeControl if set to true then the ListArray will automatically
   * reduce the internal size to save space. Setting this to false will reduce
   * heap fragmentation. Defaults to false.
   */
  ...
  /**
   * @brief Places item into given index. All other items behind given index
   * will be pushed down. If Index is larger than ListArray, then new items will
   * NOT be instantiated.
   *
  ...
  /**
   * Inserts copy of item into ListArray behind infront of the first larger
   * item.
   * @param item Item to add to ListArray.
   */
  ...
  /**
   * @brief   Will copy the items in the given list into the list. The number of
   * items to be copied is the size of the array being copied.
   * @returns reference to this array.
   */
  List<TYPE> &operator=(const ListArray<TYPE> &listB);

  /**
   * This will cut down the size of the array if it is less than half full. Can
   * cause heap fragmentation.
   */
  void reduceSize();

  /**
   * @brief Changes the size of the array. If the new size is smaller than the
   * current size, then the array will be truncated. If the new size is larger
   * than the current size, then the array will be padded with default values.
...
template <typename TYPE> void ListArray<TYPE>::reduceSize() {
...
template <typename TYPE> void ListArray<TYPE>::setSize(size_t size) {

  changeSizeTo(size);
  size_ = size;
}

template <typename TYPE> void ListArray<TYPE>::changeSizeTo(size_t size) {
...
template <typename TYPE> size_t ListArray<TYPE>::size() const { return size_; }
...
template <typename TYPE> TYPE *ListArray<TYPE>::getPtr() { return array_; }

template <typename TYPE> TYPE const *ListArray<TYPE>::getPtr() const {
  return array_;
}

template <typename TYPE> void ListArray<TYPE>::append(const TYPE &item) {
...
void ListArray<TYPE>::insert(const TYPE &item, size_t index) {
  ...
  if (index >= maxSize_) {
    ...
    while (index >= maxSize_)
      changeSizeTo(maxSize_ * 2); // Inefficient but works. Should be changed to
                                  // calculate the next largest exponent
    size_ = index;
    append(item);
  } else {
...
template <typename TYPE> bool ListArray<TYPE>::removeAtIndex(size_t index) {
...
template <typename TYPE> void ListArray<TYPE>::clear() {
