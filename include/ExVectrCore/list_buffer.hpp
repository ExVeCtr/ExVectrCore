 ...
 */
template <typename T, size_t SIZE> class ListBuffer : public List<T> {
private:
...
public:
  ...
  size_t getFront() const { return front_; }

  size_t getBack() const { return back_; }
  ...
  /**
   * Takes a element from the front of the ListBuffer and places it into
   * element. AKA dequeue item. peekFront() wont remove element. Calling this on
   * an empty ListBuffer will simply return false.
   *
  ...
  /**
   * Copies a element from the front of the ListBuffer and places it into
   * element. takeFront() will remove element. Calling this on an empty
   * ListBuffer will simply return false.
   *
   * @param element Variable whos data will be overwritten.
   * @return true if removed from ListBuffer.
   */
  ...
  /**
   * Copies a element from the back of the ListBuffer and places it into
   * element. takeBack() will remove element. Calling this on an empty
   * ListBuffer will simply return false.
   *
   * @param element Variable whos data will be overwritten.
   * @return true if removed from ListBuffer.
   ...
   */
  T getStandardDeviation(T average = getAverage()) const;

  /**
   * @returns the standard error.
   */
  T getStandardError(T average = getAverage()) const;

  /**
   * Used to access ListBuffer like an array.
   * Starts from front of ListBuffer("oldest" element or first that was placed
   * inside).
...
T ListBuffer<T, SIZE>::getStandardError(T average) const {
  if (numElements_ < 2)
    return T();
  return getStandardDeviation(average) / sqrt(numElements_);
}

template <typename T, size_t SIZE>
T ListBuffer<T, SIZE>::getStandardDeviation(T average) const {
...
template <typename T, size_t SIZE> T ListBuffer<T, SIZE>::getMedian() const {
  ...
  if (numElements_ % 2 == 0) {
    ...
    median = (ListBufferSorted[numElements_ / 2 - 1] +
              ListBufferSorted[numElements_ / 2]) /
             2;
  } else {
...
template <typename T, size_t SIZE> T ListBuffer<T, SIZE>::getAverage() const {
  if (numElements_ == 0)
    return T();
  return getSum() / numElements_;
}

template <typename T, size_t SIZE> T ListBuffer<T, SIZE>::getSum() const {
...
template <typename T, size_t SIZE> void ListBuffer<T, SIZE>::sortElements() {
...
template <typename T, size_t SIZE> void ListBuffer<T, SIZE>::clear() {
  front_ = 0;
  back_ = 0;
  numElements_ = 0;
}
...
template <typename T, size_t SIZE>
ListBuffer<T, SIZE>
ListBuffer<T, SIZE>::operator=(const ListBuffer &toBeCopied) {
...
template <typename T, size_t SIZE> size_t ListBuffer<T, SIZE>::size() const {
  return numElements_;
}

template <typename T, size_t SIZE> size_t ListBuffer<T, SIZE>::sizeMax() const {
  return SIZE;
}
...
void ListBuffer<T, SIZE>::removeFront(size_t num) {
  ...
  if (numElements_ <
      num) // If we want to remove more than we have then remove all.
    num = numElements_;
