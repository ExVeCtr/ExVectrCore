...
#define LOG_MSG(...)                                                                                            \
    {                                                                                                           \
        VCTR::Core::printD("%.6f LOG from %s, %d, %s: ", Core::NOWSeconds(), __FILE__, __LINE__, __FUNCTION__); \
...
#define VRBS_MSG(...)                                                                                               \
    {                                                                                                               \
        VCTR::Core::printD("%.6f VERBOSE from %s, %d, %s: ", Core::NOWSeconds(), __FILE__, __LINE__, __FUNCTION__); \
        VCTR::Core::printD(__VA_ARGS__);                                                                            \
    } // Use this to print debugging messages.
#else
