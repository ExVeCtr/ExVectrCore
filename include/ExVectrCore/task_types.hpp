...
public:
   ...
   * @param skipOverdueRun If true, then in the case the task has not ran for a
   * few cycles, then the task will only run once. If false then the task will
   * run the amount of times missed.
   */
  Task_Periodic(const char *taskName, int64_t interval_ns,
                int64_t start = NowNs(), int64_t timeSlip_ns = 1 * MILLISECONDS,
                bool skipOverdueRun = true);
  ...
  /**
   * Called by scheduler to allow task to do its thing. In this task type, the
   * task plans the next run.
