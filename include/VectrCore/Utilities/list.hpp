...
namespace VCTR
{
    ...
    /**
     * This is a dynamic buffer. If full, the size will be doubled. Uses heap memory and can be slower.
     * The sizeControl setting on construction can cause heap fragmentation if set to true but will reduce size.
    ...
    template <typename TYPE>
    void List<TYPE>::changeSizeTo(size_t size)
    {
    ...
    template <typename TYPE>
    inline size_t List<TYPE>::getInternalArrayLength() const
    {
        return maxSize_;
    }
    ...
    template <typename TYPE>
    void List<TYPE>::insert(const TYPE &item, size_t index)
    {
    ...
    template <typename TYPE>
    bool List<TYPE>::removeAtIndex(size_t index)
    {
