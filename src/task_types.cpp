...
VCTR::Core::Task_Periodic::Task_Periodic(const char *taskName,
                                         int64_t interval_ns, int64_t start,
                                         int64_t timeSlip_ns,
                                         bool skipOverdueRun)
    : Task(taskName) {
...
int64_t VCTR::Core::Task_Periodic::getInterval() { return interval_ns_; }

void VCTR::Core::Task_Periodic::taskRun() {
  if (skipOverdueRun_)
    deadline_ = NowNs() - NowNs() % interval_ns_ + interval_ns_;
  else
